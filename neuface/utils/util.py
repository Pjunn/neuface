...
def load_video(vid_name, frames_path, view=None):
    if not os.path.isdir(frames_path):
        os.makedirs(frames_path, exist_ok=True)
    # save_path = os.path.join(frames_path,"raw")
    ...
    while (cap.isOpened()):
        ret, frame = cap.read()
        if ret:
            # cv2.imwrite(os.path.join(save_path, 'img_%04d.jpg'%idx), frame)
            ##########If you want to cut it in square
            if w>h:
                # size = (h,h)
                # frame = frame[:,int(w/2-h/2):int(w/2+h/2)]
                if view is not None:
                    cv2.imwrite(os.path.join(save_path, 'img_%04d' % idx + '_' + view + '.jpg'), frame)
                else:
                    cv2.imwrite(os.path.join(save_path, 'img_%04d' % idx + '.jpg'), frame)
            else:
                # size = (w, w)
                # frame = frame[int(h/2-w/2):int(h/2+w/2),:]
                if view is not None:
                    cv2.imwrite(os.path.join(save_path, 'img_%04d' % idx + '_' + view + '.jpg'), frame)
                else:
                    cv2.imwrite(os.path.join(save_path, 'img_%04d' % idx + '.jpg'), frame)
            idx += 1
            ####temp###Need to be erased
            # if idx>3:
            #     break
        else:
            break
...
def vertex_normals(vertices, faces):
    ...
    # print(vertices_faces.type()) # torch.cuda.HalfTensor
    # print(faces.type()) # torch.cuda.LongTensor
    # print((faces.long()).dtype) # torch.int64
...
end_list = np.array([17, 22, 27, 31, 36], dtype=np.int32) - 1
loop_list = np.array([42, 48, 68], dtype=np.int32) - 1
...
def visualize_grid(visdict, savepath=None, size=224, dim=1, return_grid=True):
    '''
...
def create_video(img_path, out_path, fps=30):
    '''
    Creates a video from the frame format in the given directory and saves to out_path.
    '''
    command = ['ffmpeg', '-loglevel', 'quiet', '-y', '-r', str(fps), '-pattern_type', 'glob', '-i', img_path, '-c:v', 'libx264', '-crf', '21', '-pix_fmt', 'yuv420p', out_path]
    subprocess.run(command)

def point_scatter(points, color=[1.0, 0.0, 0.0]):
    ...
    if torch.is_tensor(points):
        ax.scatter(points[0, :, 0].detach().cpu().numpy(),
                   points[0, :, 1].detach().cpu().numpy(),
                   points[0, :, 2].detach().cpu().numpy(), c=color, s=0.5)

    else:
        ax.scatter(points[0, :, 0], points[0, :, 1], points[0, :, 2], c=color, s=0.5)

    ax.set_xlim(-0.15, 0.15)
    ax.set_ylim(-0.2, 0.2)
    ax.set_zlim(-0.5, 0.5)
    # ax.set_zlim(points[0, :, 2][0].item() - 1, points[0, :, 2][0].item() + 1)
