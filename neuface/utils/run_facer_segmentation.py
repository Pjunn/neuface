...
def run(seq_path : str, image, image_name : str):
    """

    :param seq_path:
    :param image: 이미지 텐서
    :param image_name: 확장자 없는 이름
    :return:
    """
    ...
    out_seg = f'{seq_path}/seg_og/'
    out_seg_annot = f'{seq_path}/seg_non_crop_annotations/'
    ...
    out_cropped = f'{seq_path}/cropped/'
    os.makedirs(out_cropped, exist_ok=True)


    # 지금 입력은 (batch, channel, width, height)
    # 이미지 이름이 담긴
    ...
    for i in range(len(image)//batch_size):
        image_stack = []
        frame_stack = []
        original_shapes = []
        for j in range(batch_size):
            # file = frames[i * batch_size + j]
            file = image_name[i * batch_size + j]

            if os.path.exists(f'{out_seg_annot}/color_{file}.png'):
                # print('DONE')
                continue
            # img = Image.open(f'{folder}/{file}')#.resize((512, 512))
            img = image[i * batch_size + j]
            og_size = img.size
            # cropped image save
            if not os.path.exists(f'{out_cropped}/{file}.png'):
                img_np = (img[0].permute(1,2,0).numpy()).astype(np.uint8)
                I = Image.fromarray(img_np)
                I.save(f'{out_cropped}/{file}.png')
        ...
        for batch_idx in range(ceil(len(image_stack)/batch_size)):
            ...
            try:
                ...
                seg_classes = seg_probs.argmax(dim=1).detach().cpu().numpy().astype(np.uint8) # nfaces x h x w
