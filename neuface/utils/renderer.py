...
class SRenderY(nn.Module):
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
        ...
        # # rotate to front view - not working
        # vertices, transformed_vertices = util_s.rotate_vertices_single(vertices, transformed_vertices, ref_vertices, ref_transformed_vertices)
        ...
        if colors is None:
            colors = self.face_colors.expand(batch_size, -1, -1, -1) # (7, 9976, 3, 3)
        else:
            colors = util.face_vertices(colors, self.faces.expand(batch_size, -1, -1))
