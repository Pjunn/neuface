        ...
        for i in tqdm(range(len(image_names))):
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    #TODO output detach for visualization
                    # detached_output = output['uv_map'][0,0]
                    # detached_output = pad_to_3_channels(detached_output.permute(1, 2, 0).detach().cpu().float().numpy())
                    # detached_output_mirrored = output_mirrored['uv_map'][0,0]
                    # detached_output_mirrored = pad_to_3_channels(detached_output_mirrored.permute(1, 2, 0).detach().cpu().float().numpy())
                    if 'uv_map' in output:
                    ...
                    # detached_output = pad_to_3_channels(output['uv_map'][0,0].permute(1, 2, 0).detach().cpu().float().numpy())
                ...
                for prediction_type in prediction_types:
                    for i_batch in range(batch_size):
                        ...
                        if prediction_type == 'uv_map':
                            ...
                            # detached_tmp_output = pad_to_3_channels(tmp_output.permute(1, 2, 0).detach().cpu().float().numpy())
                            ...
                            # detached_tmp_output = tmp_output.permute(1, 2, 0).detach().cpu().float().numpy()
...
if __name__ == '__main__':
    base_conf = OmegaConf.load(f'/configs/base.yaml')
